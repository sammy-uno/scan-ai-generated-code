...
def get_live_pr_status(repo, pr_num):
    if not repo or not pr_num: return "🟣 Merged"
    try:
...
def main():
    ...
    # 🚀 LOCAL TEST ACCUMULATION CACHE FILE
        ...
        try:
            with open(accumulated_db_path, "r", encoding="utf-8") as db_f:
                ...
                if isinstance(historical_rows, list):
                    table_rows.extend(historical_rows)
                    for r in historical_rows:
                        seen_pr_keys.add(f"{r.get('repo')}#{r.get('link')}")
                    print(f"📥 [TEST SYNC] Loaded {len(historical_rows)} prior records from history.")
        except Exception as db_err:
            print(f"⚠️ Failed to ingest historical test database: {db_err}")
    ...
    for f in success_markers:
        ...
        try:
            ...
            link_md = f'[#{pr_num}]({base_domain}/{clean_repo_path}/pull/{pr_num})'
            ...
            if target_json_path and os.path.exists(target_json_path):
                with open(target_json_path, 'r', encoding='utf-8') as sm_f:
                    ...
                    cwe_display = ', '.join(sorted(list(set(str(c).strip().upper() for c in cwes_list if c)))) if isinstance(cwes_list, list) else "None"
            else:
                cwe_display = "None"

            cwe_density = round(total_issues / live_loc, 4) if live_loc > 0 and committed_files_count > 0 else 0.0
