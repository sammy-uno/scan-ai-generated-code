...
def main():
    ...
    <script>
        ...
        function sortCweColumn(headerElement) {{
            ...
            // Find the visual span text element inside the clicked header
            const indicator = headerElement.querySelector('.sort-indicator');
        
            cweSortToggleState = !cweSortToggleState;

            // 🎯 DYNAMIC INDICATOR: Change the arrows based on the sorting state
            if (indicator) {{
                indicator.innerText = cweSortToggleState ? '▲' : '▼';
                indicator.style.color = '#f59e0b'; // Optional: Highlight indicator color on active sort
            }}
    ...
    <table><thead><tr><th>Security Alert Status</th><th>Repository Target</th><th>PR Reference Link</th><th>Status</th><th>AI Tool Engine</th><th>Language</th><th>LOC</th><th onclick="sortCweColumn(this)" style="cursor: pointer; user-select: none;"><span class="sort-indicator" style="margin-left: 5px; color: #888;">↕</span>CWE Discovered</th><th>🔴 H</th><th>🟡 M</th><th>🔵 L</th><th>Total Issues (Files)</th></tr></thead><tbody id="tableBodyContainer">"""
