...
def main():
    ...
    for filepath in downloaded_slices:
        ...
        try:
            with open(filepath, "r", encoding="utf-8") as f_slice:
                ...
                # 🎯 FIXED: Explicitly strip any accidental double-dashes or punctuation artifacts
                repo_raw = str(parts).strip("- ")
                repo_clean = repo_raw.replace("_SLASH_", "/")
                pr_clean = str(parts).strip("- ")
                
                lang_clean = parts if len(parts) > 2 else "Unknown"
                tool_clean = parts.replace("_", " ") if len(parts) > 3 else "Static Tool"
                
                raw_size = parts if len(parts) > 4 else "100"
                ...
                print(f"     ├── Repo correlated: {repo_clean} | PR Number: #{pr_clean} | Scan Engine: {tool_clean}")
    ...
    for s_path in sarif_logs:
        ...
        # 🎯 FIXED: Enforce matching index cleaning parameters
        repo_raw = str(parts).strip("- ")
        repo_clean = repo_raw.replace("_SLASH_", "/")
        pr_clean = str(parts).strip("- ")
        tool_clean = parts.replace("_", " ") if len(parts) > 3 else "Static Tool"
