...
def main():
    ...
    <script>
        ...
        function sortCweColumn(headerElement) {{
            ...
            // Separate main data rows from detail rows
            const mainRows = [];
            ...
            // Sort only the main rows using Index 7
            mainRows.sort((rowA, rowB) => {{
                // 🎯 FIXED: Explicitly look at Index 7 to find the CWE column text
            ...
            // Re-append to the table body container
            tableBody.innerHTML = "";
            mainRows.forEach((mainRow) => {{
                tableBody.appendChild(mainRow);
                // Look up the matching details row that sat right beneath it in the original rowsArray
